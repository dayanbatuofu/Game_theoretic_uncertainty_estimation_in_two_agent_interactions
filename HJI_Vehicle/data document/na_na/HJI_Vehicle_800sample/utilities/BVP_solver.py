...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,
    ...
    while True:
        ...
        # alpha = 1e-6
        # for i in range(m):
        #     if y[0, i]>34.25 and y[0, i]<38.75 and y[2, i]>34.25 and y[2, i]<38.75 and collision==False:
        #         Istart = i
        #         Tstart = x[Istart]
        #         collision = True
        #     if collision==True and (y[0, i]>38.75 or y[2, i]>38.75):
        #         Iend = i
        #         Tend = x[Iend]
        #         collision = False
        #         Ti = Tend - Tstart
        # if not Tstart==0:
        #     # y[4, Istart:Iend+1] = 10 ** 4 / y[1, Iend]
        #     # y[10, Istart:Iend+1] = 10 ** 4 / y[3, Iend]
        #     y[4, Istart:Iend + 1] = 10 ** 4
        #     y[10, Istart:Iend + 1] = 10 ** 4
        #     S1, S2 = y[5, Iend], y[11, Iend]
        #     for i in range(Istart, Iend):
        #         # y[5, i] = -10 ** 4 / y[1, Iend] * (x[i] - Tend) + S1
        #         # y[11, i] = -10 ** 4 / y[3, Iend] * (x[i] - Tend) + S2
        #         y[5, i] = -10 ** 4 * (x[i] - Tend) + S1
        #         y[11, i] = -10 ** 4 * (x[i] - Tend) + S2
        #     for i in range(Istart):
        #         # y[5, i] = -(10 ** 4 / y[1, Iend] - 10 ** 4 / y[1, Istart] + alpha) * (x[i] - Tstart) + S1 + 10 ** 4 / y[1, Iend] * Ti
        #         # y[11, i] = -(10 ** 4 / y[3, Iend] - 10 ** 4 / y[3, Istart] + alpha) * (x[i] - Tstart) + S2 + 10 ** 4 / y[3, Iend] * Ti
        #         y[5, i] = -alpha * (x[i] - Tstart) + S1 + 10 ** 4 * Ti
        #         y[11, i] = -alpha * (x[i] - Tstart) + S2 + 10 ** 4 * Ti
