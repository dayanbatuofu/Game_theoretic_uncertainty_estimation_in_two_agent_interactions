...
from HJI_Vehicle.utilities.neural_networks import HJB_network as HJB_network
from HJI_Vehicle.NN_output import get_Q_value
from HJI_Vehicle_costate.NN_output_costate import get_Hamilton, get_Costate
import dynamics
import pdb
...
class InferenceModel:
    def __init__(self, model, sim):  # model = inference type, sim = simulation class
        ...
        elif model == 'empathetic':  # uses nfsp
                ...
                for j in range(len(p_betas_d[i])):
                    ...
                    for id in range(self.sim.n_agents):
                        p_a_i = bvp_action_prob_2(id, past_state_h, past_state_m,
                                                  ...
                                                  last_actions[id-1], self.time-self.dt)  # last time step
    ...
    def bvp_opti_inference(self, agent, sim):
        ...
        def q_func_integrate_helper1(Ui, X, T, last_u_other, theta, lambda_1, deltaT):
            last_action = 0
            q1, q2 = get_Q_value(X, T, np.array([[Ui], [last_u_other]]), theta)
        ...
        def q_func_integrate_helper2(Ui, X, T, last_u_other, theta, lambda_2, deltaT):
            last_action = 0
            q1, q2 = get_Q_value(X, T, np.array([[last_u_other], [Ui]]), theta)
        ...
        def action_prob_continuous(id, p1_state, p2_state, beta_1, beta_2, last_action, time):
            ...
            time = np.array([[time]])
            q1, q2 = get_Q_value(x1_nn, time, last_actions_nn, theta)
        ...
        def bvp_optimized_action(id, p1_state, p2_state, beta_1, beta_2, last_action, time):
            ...
            def q_function_helper1(action):  # for agent 1
                ...
                new_s2_last = dynamics.bvp_dynamics_1d(p2_state, last_action, dt)  # new state of p2 using last u
                # if (theta_1, theta_2) == (1, 5):
                ...
                # else:
                new_x1_nn = np.array([[new_p1_s[1]], [new_p1_s[3]], [new_s2_last[0]], [new_s2_last[2]]])
                q1, q2 = get_Q_value(new_x1_nn, time, np.array([[action], [last_action]]),
                                     (beta_1[0], beta_2[0]))
                q1 = -q1[0][0]
                # print(action, q1)
                return q1
            ...
            def q_function_helper2(action):  # for agent 2
                ...
                new_s1_last = dynamics.bvp_dynamics_1d(p1_state, last_action, dt)  # new state of p1 using last u
                # if (theta_1, theta_2) == (1, 5):
                #     new_x2_nn = np.array([[new_p2_s[0]], [new_p2_s[2]], [new_p1_s_last[1]], [new_p1_s_last[3]]])
                #     q2, q1 = get_Q_value(new_x2_nn, time, np.array([[last_a_2], [action]]),
                #                          (true_beta_m[0], true_beta_h[0]), dt)
                # else:
                new_x1_nn = np.array([[new_s1_last[1]], [new_s1_last[3]], [new_p2_s[0]], [new_p2_s[2]]])
                q1, q2 = get_Q_value(new_x1_nn, time, np.array([[action], [last_action]]),
                                     (beta_1[0], beta_2[0]))
                q2 = -q2[0][0]
                # print(action, q2)
        ...
        # print("-Intent_inf- marginal state H: ", marginal_state_h)
        return {
            # 'predicted_states_other': (marginal_state_h, get_state_list(curr_state_h, self.T, self.dt)),  # col of 2D should be H
            'predicted_actions_other': predicted_actions[1],
            'predicted_intent_other': [p_beta_d_m, new_beta_2],
            # 'predicted_states_self': (marginal_state_m, get_state_list(curr_state_m, self.T, self.dt)),
            'predicted_actions_self': predicted_actions[0],
