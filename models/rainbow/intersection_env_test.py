...
class IntersectionEnv:
    def __init__(self, control_style_ego, control_style_other, time_interval, max_time_steps):
        ...
        self.max_time_steps = 200 #max_time_steps
    ...
    def step(self, action):
        ...
        for t in range(int(self.time_interval / self.min_time_interval + 1)):
            ...
            self.ego_car.state = [x_ego_new, v_ego_new]
            self.other_car.state = [x_other_new, v_other_new]
            if self.args.render:
                self.render()
            if self.is_recording:
                pg.image.save(self.renderer.screen, "%simg%03d.png" % (self.recording_path, self.total_frames))
        ...
        if self.args.acc_loss:
            # print(action, action_self, action_other)
    ...
    def mpc_psuedo_batch(self, args, state, q_set, pos_trajectories, T):
        ...
        if args.requires_grad:
            ...
            traj_tensor = torch.tensor(pos_trajectories, dtype=torch.float, device=args.device)
