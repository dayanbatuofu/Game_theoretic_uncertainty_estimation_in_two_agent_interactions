...
from HJI_Vehicle_costate.examples.choose_problem import system, problem, config
import copy
...
def solve_newton(n, m, h, col_fun, bc, jac, y, p, B, bvp_tol, bc_tol):
    ...
    for iteration in range(max_iter):
        if recompute_jac:
            ...
            step = LU.solve(res)
            # step = spsolve(J, res, permc_spec='MMD_ATA')
...
def solve_bvp2(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,
    ...
    while True:
        ...
        '''
        This is the new code we implement to update the lambda_11 and lambda_22 when this is collision case
        '''
        N = x.shape[0]
        collision = False
        symmetric = False
        tmp_lambda = np.zeros((14, N))

        for i in range(N):
            tmp_lambda[0, i] = copy.deepcopy(y[0, i])
            tmp_lambda[1, i] = copy.deepcopy(y[1, i])
            tmp_lambda[2, i] = copy.deepcopy(y[2, i])
            tmp_lambda[3, i] = copy.deepcopy(y[3, i])
            tmp_lambda[4, i] = copy.deepcopy(y[4, i])
            tmp_lambda[5, i] = copy.deepcopy(y[5, i])
            tmp_lambda[6, i] = copy.deepcopy(y[6, i])
            tmp_lambda[7, i] = copy.deepcopy(y[7, i])
            tmp_lambda[8, i] = copy.deepcopy(y[8, i])
            tmp_lambda[9, i] = copy.deepcopy(y[9, i])
            tmp_lambda[10, i] = copy.deepcopy(y[10, i])
            tmp_lambda[11, i] = copy.deepcopy(y[11, i])
            tmp_lambda[12, i] = copy.deepcopy(y[12, i])
            tmp_lambda[13, i] = copy.deepcopy(y[13, i])
        ...
        position_line = y[2, 0] / y[0, 0]

        for i in range(N):
            if y[0, i] > collision_lower1 and y[0, i] < collision_upper1 and y[2, i] > collision_lower2 and \
                    y[2, i] < collision_upper2 and collision == False:
                Istart = i
                Tstart = x[Istart]
                collision = True
            if collision == True and (y[0, i] > collision_upper1 or y[2, i] > collision_upper2):
                Iend = i
                Tend = x[Iend]
                collision = False
                Ti = Tend - Tstart

            if boundary_line >= position_line and y[0, i] > collision_upper2 and collision == False:
                Istart = i
                Tstart = x[Istart]
                collision = True
            if boundary_line < position_line and y[2, i] > collision_upper1 and collision == False:
                Istart = i
                Tstart = x[Istart]
                collision = True
        ...
        if not Tstart == 0:
            ...
            y[10, Istart:N] = alpha
            y[5, -1] = -2 * (y[1, -1] - y[1, 0])
            y[11, -1] = -2 * (y[3, -1] - y[3, 0])

            if boundary_line <= position_line:
                y[4, 0:Istart] = 10 ** 4 / y[1, Istart]
                y[10, 0:Istart] = -10 ** 4 / y[3, Istart]
                k1 = 10 ** 4 / y[1, Istart]
                k2 = -10 ** 4 / y[3, Istart]
                for i in range(Istart, N - 1):
                    y[5, i] = -alpha * (x[i] - x[-1]) + y[5, -1]
                    y[11, i] = -alpha * (x[i] - x[-1]) + y[11, -1]
                S1, S2 = y[5, Istart], y[11, Istart]
                for i in range(Istart):
                    y[5, i] = -k1 * (x[i] - Tstart) + S1
                    y[11, i] = -k2 * (x[i] - Tstart) + S2
