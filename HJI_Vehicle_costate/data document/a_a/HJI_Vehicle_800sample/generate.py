...
if data_type:
    ...
    for m in range(11):  # segment for position x2, separate into 11 segment for train data
        for n in range(11):  # segment for position x1, separate into 11 segment for train data
            ...
            if X_guess[0, 0] >= X_guess[2, 0]:
                if t1 < t2:
                    ...
                    X_guess1[0, -2] = collision_upper
                    X_guess1[2, -2] = X_guess1[2, 0] + X_guess1[3, 0] * t_guess1[-2] - 2.5 * t_guess1[-2] ** 2
                    X_guess1[3, -2] = X_guess1[3, 0] - 5 * t_guess1[-2]
                else:
                    for i in range(1, n_sample + 2):
                        X_guess1[0, i] = X_guess1[0, 0] + X_guess1[1, 0] * t_guess1[i]
                ...
                if t1 < t2:
                    ...
                    X_guess1[0, -2] = X_guess1[0, 0] + X_guess1[1, 0] * t_guess1[-2] - 2.5 * t_guess1[-2] ** 2
                    X_guess1[1, -2] = X_guess1[1, 0] - 5 * t_guess1[-2]
                    X_guess1[2, -2] = collision_upper
                else:
                    for i in range(1, n_sample + 2):
    ...
    X0 = X0_pool[:, 0]
    X0_orignal = copy.deepcopy(X0)
    for m in range(6):  # segment for position x2, separate into 6 segment for validation data
        for n in range(6):  # segment for position x1, separate into 6 segment for validation data
            print('Solving BVP #', N_sol + 1, 'of', Ns, '...', end='\r')
