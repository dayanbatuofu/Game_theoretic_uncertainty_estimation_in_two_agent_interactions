...
class AutonomousVehicle:
    ...
    def update(self, frame):
        ...
        wanted_others_prediction_of_my_trajectory = \
    ...
    def multi_search(self, guess_set):
        ...
        grace = 0.
        for wanted_trajectory_self in s.wanted_trajectory_self:
            grace += (trajectory[0] - wanted_trajectory_self[0]) ** 2
        s.social_gracefulness.append(grace/len(s.wanted_trajectory_self))
        return trajectory
    ...
    def multi_search_intent(self):
        ...
        for theta_self in trials_theta:
            for theta_other in trials_theta:
                ...
                other_trajectory = [trajectory_other[i] for i in
                                    np.where(my_loss_all == np.min(my_loss_all))[0]]  # others belive I move fast
                ...
                if trajectory_self is not []:
                    ...
                    trajectory_other = np.atleast_2d([other_trajectory_conservative[i] for i in
                                                      np.where(fun_self == np.min(fun_self))[0]])
                ...
                # id1 = np.random.randint(len(trajectory_other))
                # id2 = np.random.randint(len(trajectory_self))
        ...
        return theta_other_out, theta_self_out, trajectory_other_out, trajectory_self_out, trajectory_self_out
        ...
        for theta_self in trials_theta:
            for theta_other in trials_theta:
                ...
                if trajectory_self is not []:
                    ...
                    trajectory_self_wanted_other = \
                        [trajectory_self_wanted_other[i] for i in np.where(fun_other == np.min(fun_other))[0]]

                    trajectory_self = [my_trajectory[i] for i in np.where(fun_self == np.min(fun_self))[0]]
                    trajectory_other = [other_trajectory_conservative[i] for i in
                                        np.where(fun_self == np.min(fun_self))[0]]
                    # my_loss_all = [my_loss_all[i] for i in np.where(fun_self == np.min(fun_self))[0]]
                    #
                    # trajectory_self = [trajectory_self[i] for i in np.where(my_loss_all == np.min(my_loss_all))[0]]
        ...
        for theta_self in trials_theta:
            for theta_other in trials_theta:
                ...
                if trajectory_self is not []:
                    ...
                    # what I think I should do from all equilibrium
                    trajectory_self = [my_trajectory[i] for i in np.where(fun_self == np.min(fun_self))[0]]
                    # what I think other should do if I do trajectory_self
                    trajectory_other = [other_trajectory_conservative[i] for i in
                                        np.where(fun_self == np.min(fun_self))[0]]
